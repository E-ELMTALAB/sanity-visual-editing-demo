...
export class ProductSyncService {
  ...
  /**
   * Sync product variants (options) to Medusa
   */
  async syncProductVariants(sanityProduct: ProductDoc): Promise<void> {
    const { medusaProductId, options } = sanityProduct

    if (!medusaProductId || !options || options.length === 0) {
      return
    }

    try {
      // Get existing variants
      const { product } = await this.medusaClient.admin.products.retrieve(medusaProductId)
      const existingVariants = product.variants || []

      // Create variants for each option
      for (const option of options) {
        // Check if variant already exists
        const existingVariant = existingVariants.find((v) => v.sku === option.id)

        if (!existingVariant) {
          await this.medusaClient.admin.products.createVariant(medusaProductId, {
            title: option.name || 'Option',
            sku: option.id || `variant-${Date.now()}`,
            manage_inventory: false,
            allow_backorder: true,
            prices: [
              {
                amount: (option.price || 0) * 10000, // Convert to smallest unit (rials)
                currency_code: 'irr',
              },
            ],
          })
  ...
  /**
   * Sync product prices to Medusa
   */
  async syncProductPrices(sanityProduct: ProductDoc, regionId: string = 'reg_iran'): Promise<void> {
    const { medusaProductId, price, originalPrice } = sanityProduct
    ...
    try {
      ...
      // Update price for default variant
      const defaultVariant = variants[0]
      
      // Note: Price updates require a more complex API call
      // This is a placeholder - actual implementation depends on Medusa version
