...
function EditorialBannerItem({ banner, index, isRTL }: { banner: Banner; index: number; isRTL: boolean }) {
  const { src: resolvedBannerImage, onError: onBannerImageError } = useImageFallback({
    imageKey: banner.id || banner.title,
    sanityUrl: banner.backgroundImage,
  });
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        <img
          src={resolvedBannerImage || banner.backgroundImage}
      ...
      <div className="max-w-screen-xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
          {banners.map((banner, index) => (
            <EditorialBannerItem key={banner.id} banner={banner} index={index} isRTL={isRTL} />
          ))}
        </div>
      </div>
    </section>
