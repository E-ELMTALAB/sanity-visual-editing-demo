...
// Default countdown duration: 48 hours from first view
const DEFAULT_DURATION_MS = 48 * 60 * 60 * 1000;

function getOrInitExpiry(): string {
  if (typeof window === "undefined") return new Date(Date.now() + DEFAULT_DURATION_MS).toISOString();
...
export function PromoBanner({ className }: PromoBannerProps) {
  ...
  useEffect(() => {
    ...
    try {
      const dismissedStored = window.localStorage.getItem(DISMISS_KEY);
    ...
    <section className={cn("py-8 sm:py-10 lg:py-12 px-6 sm:px-6 lg:px-[100px]", className)}>
      <div className="max-w-[1400px] mx-auto">
        ...
        >
          ...
          {/* Content */}
          <div className="relative z-10 flex w-full flex-col sm:flex-row items-center sm:items-center justify-between gap-6 px-6 sm:px-10 lg:px-12 py-8 sm:py-10 lg:py-12">
            {/* Text + badge */}
            <div className="flex-1 flex flex-col items-center sm:items-end text-center sm:text-right">
              ...
              >
                <div className="flex items-center gap-1.5 px-3 py-1">
                  <Sparkles className="w-[14px] h-[14px] text-amber-300" />
                  <span className="font-vazirmatn text-[12px] font-bold tracking-wide text-amber-300">
                    پیشنهاد محدود زمانی
                  </span>
                </div>
              </motion.div>

              <h2 className="mt-3 sm:mt-3 lg:mt-4 font-vazirmatn font-black text-[20px] sm:text-[24px] lg:text-[30px] leading-tight tracking-tight text-white">
                تخفیف ویژه روی اکانت‌های پریمیوم ChatGPT
              </h2>

              <p className="mt-2 sm:mt-2 lg:mt-2 font-vazirmatn text-[14px] sm:text-[16px] font-medium text-white/70 max-w-xl">
                فقط تا پایان شمارش معکوس، می‌توانید اکانت‌های قانونی با تحویل آنی و پشتیبانی واقعی را
                با قیمت ویژه تهیه کنید.
              </p>
            </div>

            {/* Right side: CTA + countdown */}
            <div className="flex flex-col items-center sm:items-end gap-4 min-w-[200px]">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.98 }}
                className="group relative inline-flex items-center justify-center gap-2 overflow-hidden rounded-full border border-white/20 px-5 sm:px-6 lg:px-7 py-2.5 shadow-[0_4px_24px_rgba(147,51,234,0.4)]"
                style={{
                  backgroundImage: "linear-gradient(90deg,#7C3AED,#EC4899,#7C3AED)",
                  backgroundSize: "200% 100%",
                  backgroundPosition: "0% 0%",
              ...
              >
                ...
                <motion.span
                  ...
                  className="flex items-center justify-center"
                >
                  <ArrowLeft className="w-[16px] h-[16px] sm:w-[20px] sm:h-[20px] text-white" />
                </motion.span>
